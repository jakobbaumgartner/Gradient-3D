...
\comment{}{PLOT: POI on the ROBOT + ONE OR MORE KERNELS ON THE ROBOT}
...
The fundamental concept of our directional kernels lies in computing the repulsive field individually for each direction within the Cartesian coordinate system. Our filters can be likened to a variation of the Sobel \add{reference} operator, a 2D convolutional \add{reference} filter frequently utilized in computer vision for calculating image gradients at specific points. However contrary to the Sobel operator, our filter is not calculating the gradients of the matrix, as the inputs are weighted depending on the distance from the center of the kernel. 
...
Firstly we use our kernel matrix multiplication method, to calculate the repulsive velocity values for the eight surrounding cells that are closest to the chosen POI. We get the indexes of the surrounding cells by first scaling the POI position by grid resolution and than rounding the position to the nearest lower and upper integer positions.
...
\begin{equation}
	\vec{V}rep_{xyz} = \mathrm{calc\_rep\_vel}(X[i], Y[j], Z[k]) \quad \forall i, j, k \in \{1, 2\}
\end{equation}

%\begin{equation}
%	\Delta X = \frac{\left( P_x - X(1) \right)}{\left( X(2) - X(1) \right)}
%\end{equation}
%
%\begin{equation}
%	\Delta Y = \frac{\left( P_y - Y(1) \right)}{\left( Y(2) - Y(1) \right)}
%\end{equation}
%
%\begin{equation}
%	\Delta Z = \frac{\left( P_z - Z(1) \right)}{\left( Z(2) - Z(1) \right)}
%\end{equation}
...
\begin{equation}
	...
	=
	\begin{bmatrix}
		 \frac{\left( P_x - X(1) \right)}{\left( X(2) - X(1) \right)} \\
		 \frac{\left( P_y - Y(1) \right)}{\left( Y(2) - Y(1) \right)} \\
		 \frac{\left( P_z - Z(1) \right)}{\left( Z(2) - Z(1) \right)} \\
 	\end{bmatrix}
\end{equation}
...
\comment{}{PLOT: surrounding cells}

Once we have identified these cells, we apply our established kernel matrix method to compute the x, y, and z components of the repulsive velocity vector for each of the cells.

Subsequently, we separately perform trilinear interpolation for each direction of the repulsive velocity vector. 

It is important to consider that the calculated repulsive velocity values are located at the centers of the cells. Therefore, during interpolation, we shift the values of the cells coordinates by half the resolution of the obstacle grid for each direction. This results in a repulsive velocity vector that transitions smoothly between the discrete values calculated at distinct points in the obstacle grid.



\add{enačbe trilinearne interpolacije}
