...
Firstly, from the coordinates of our POI, we calculate the coordinates of the eight surrounding cells in the obstacle grid, by rounding the coordinates up and down in x, y, and z directions.
...
Subsequently, we separately perform trilinear interpolation for each direction of the repulsive velocity vector. It is important to consider that the calculated repulsive velocity values are located at the centers of the cells. Therefore, during interpolation, we shift the values of the cells coordinates by half the resolution of the obstacle grid for each direction. This results in a repulsive velocity vector that transitions smoothly between the discrete values calculated at distinct points in the obstacle grid.
