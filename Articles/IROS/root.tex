...
Robots need to quickly adapt to changes in dynamic environment, especially when obstacles appear. Our method helps a robot calculate safe directions away from obstacles by looking at the space around it in real-time. The focus is on a set area around the robot.

In 3D graphics, a voxel is like a tiny cube in a grid that makes up the space. We use voxels to map the workspace of the robot, each voxel has a value that tells us if it’s probably empty or occupied. A zero means empty, and a one means definitely something is there, with values in between showing varying levels of certainty.
...
The grid of voxels can be set in advance to match the layout of the space. We can also change the values in the voxels if we know where other objects or agents are moving. Besides pre-setting, we can use sensors to detect what's around.

Our method calculates repulsive velocities, which are vectors that guide the robot on where not to go by pointing away from potential collisions. These vectors gain strength as the robot approaches an obstacle, signaling an urgent need to maneuver to safety. Conversely, when the robot is positioned at an equal distance from obstacles in the vicinity—safely away from all potential collisions—the repulsive velocities weaken because there’s no immediate need to move. 
