function [f] = showMovementPandaChangingGrid(Tbase, control_points, output)
    ...
    % create grid object to get to the methods
    grid = OctoGrid(2,2,2,10);
    
    % display grid
    HObstacles = grid.showGridVol3D(output.grids{end},'floor',floor,'height',true);
    hold on
    axis equal
...
%         plotTransforms(transforms(1:3, 4, i)'*grid.resolution, rotm2quat(transforms(1:3, 1:3, i)), 'FrameSize', 0.2);
    ...
    %         plotTransforms(transforms(1:3, 4, i)'*grid.resolution, rotm2quat(transforms(1:3, 1:3, i)), 'FrameSize', 0.2);
        ...
        if exist('HObstacles', 'var') && isfield(HObstacles, 'handles')
            for i = 1:length(HObstacles.handles)
                delete(HObstacles.handles(i));
            end
        end

        HObstacles = grid.showGridVol3D(output.grids{j},'floor',floor,'height',true);

        view(current_azimuth, current_elevation);
