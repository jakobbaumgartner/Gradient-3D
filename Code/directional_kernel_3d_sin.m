function [kernel3D] = directional_kernel_3d_sin(direction, kernel_length, sigma, kernel_width, kernel_height, kernel_type)

   % SETTINGS
   % ---------------------------------------------------------------------
   constant_kernel_extensions = 0;
...
%         kernel3D = repmat(kernel2D, 1, 1, length(height_extender)) .* reshape(height_extender, 1, 1, []);
   ...
   % EXTEND KERNELS FOR WIDTH AND HEIGHT
   % ---------------------------------------------------------------------

   % extend weights to kernel width
   center_width = floor(kernel_width/2);
   
   % if constant_kernel_extensions, then use a constant weight for the width
   if (constant_kernel_extensions)
       width_extender = [ones(1,center_width) 1 ones(1,center_width)];
   else
       width_extender = sin((0:center_width)*pi/2/center_width)
       width_extender = [width_extender 1 flip(width_extender)];
   end
   ...
   % if constant_kernel_extensions, then use a constant weight for the height
   if (constant_kernel_extensions)
       height_extender = [ones(1,center_height) 1 ones(1,center_height)];
   else
       height_extender = sin((0:center_height)*pi/2/center_height)
       height_extender = [height_extender 1 flip(height_extender)];
   end
