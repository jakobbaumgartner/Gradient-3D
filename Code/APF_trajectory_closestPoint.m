    ...
    % --------------------------------------------------
 	if avoid_task
        ...
        for i = 1:1:length(transformations_list) % for every detection point
            ...
            % calculate derivative of APF
            [dx,dy,dz] = interpolate_derivative( transform(1:3,4)', grid_repulsive, space_resolution);

            APF_save = [APF_save ; transform(1:3,4)' dx dy dz];
        end
