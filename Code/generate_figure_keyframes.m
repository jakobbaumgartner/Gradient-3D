function [f] = generate_figure_keyframes(grid, Tbase, control_points, output)
    ...
    for kf = 1:length(keyframes)
        ...
        % Apply custom ticks and labels for X, Y, and Z axes
        customTicks = linspace(0, 2, 5); % 0, 0.5, 1, 1.5, 2
        customTicksZ = linspace(0,1,5);
        % Adjusting scale to match axis limits
        scaleX = linspace(5, 20, numel(customTicks));
        scaleY = linspace(5, 15, numel(customTicks));
        scaleZ = linspace(0, 10, numel(customTicks)); % For Z-axis in side-view

        set(gca, 'XTick', scaleX, 'XTickLabel', arrayfun(@num2str, customTicks, 'UniformOutput', false));
        set(gca, 'YTick', scaleY, 'YTickLabel', arrayfun(@num2str, customTicks, 'UniformOutput', false));
        set(gca, 'ZTick', scaleZ, 'ZTickLabel', arrayfun(@num2str, customTicksZ, 'UniformOutput', false));
