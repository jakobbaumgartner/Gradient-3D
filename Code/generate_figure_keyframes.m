function [f] = generate_figure_keyframes(grid, Tbase, control_points, output)
    ...
    for kf = 1:length(keyframes)
        ...
        set(gca, 'XTick', linspace(0, 20, 5), 'XTickLabel', {'0', '0.5', '1', '1.5', '2'});
        set(gca, 'YTick', linspace(0, 20, 5), 'YTickLabel', {'0', '0.5', '1', '1.5', '2'});
        % Adjust Z-ticks to represent 0 to 1 in steps of 0.5, mapped to 0 to 10
        set(gca, 'ZTick', [0, 5, 10], 'ZTickLabel', {'0', '0.5', '1'});
...
%         grid.showGridVol3D(grid.grid, 'floor', false, 'height', true);
        grid.showGridVoxel(grid.grid, 'floor', false, 'height', true);
        ...
        % Adjust the view for each subplot if needed
        view([-180 45.0]);
        view([-190, 45])
