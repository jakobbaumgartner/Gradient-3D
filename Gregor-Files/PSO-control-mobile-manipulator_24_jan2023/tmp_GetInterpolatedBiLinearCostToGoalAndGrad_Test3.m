...
function rrr=tmp_GetInterpolatedBiLinearCostToGoalAndGrad_Test3(xx,yy,FF,maxFF,space,resolution)
   ...
   flagInterpolatedGradeint=1;
   if(flagInterpolatedGradeint)
      ...
      % doloci gradiente centrov celic za vse 4 celice
       ...
       loc=00; % v kateri celici se nahajam
       if xx <= xs && yy<=ys % spodnji leva cetrtina celice       
            x0= ix-1; x1=ix; y0=iy-1;  y1=iy;  xx0=xs-eC; xx1=xs;       yy0=ys-eC; yy1=ys;   loc=11;
       elseif xx > xs && yy<=ys % spodnji desna cetrtina celice
            x0=ix; x1=ix+1; y0=iy-1;  y1=iy;   xx0=xs;       xx1=xs+eC; yy0=ys-eC; yy1=ys;   loc=01;
       elseif xx > xs && yy>ys % zgornja desna cetrtina celice
            x0=ix; x1=ix+1; y0=iy;  y1=iy+1;   xx0=xs;       xx1=xs+eC; yy0=ys;    yy1=ys+eC;loc=00; 
       elseif xx <= xs && yy>ys % zgornja leva cetrtina celice
            x0=ix-1; x1=ix; y0=iy;  y1=iy+1;   xx0=xs-eC; xx1=xs;       yy0=ys;    yy1=ys+eC;loc=10;  
       end
   ...
   %    [g00,g01,g10,g11]=getCellCenterGradients2(xx0,yy0,xx1,yy1,ix,iy,FF,maxFF,eC,x_min,x_max,y_min,y_max,sizeX,sizeY);
       ...
       ax=x0; ay=y0; 
       g00=getCellGrad(ax,ay,x0,y0,xx,yy,rrr, FF,sizeX,sizeY,x_min,y_min,eC,maxFF);
       ax=x0; ay=y1;
       g01=getCellGrad(ax,ay,x0,y0,xx,yy,rrr, FF,sizeX,sizeY,x_min,y_min,eC,maxFF);
       ax=x1; ay=y0;
       g10=getCellGrad(ax,ay,x0,y0,xx,yy,rrr, FF,sizeX,sizeY,x_min,y_min,eC,maxFF);
       ax=x1; ay=y1;
       g11=getCellGrad(ax,ay,x0,y0,xx,yy,rrr, FF,sizeX,sizeY,x_min,y_min,eC,maxFF);

             
     % test uredim diagonalne prehode mimo dotika zasedenih celic
    
   if x0>0 && y0>0   
     if(FF(x0,y0)>maxFF && FF(x1,y1)>maxFF && FF(x0,y1)<maxFF && FF(x1,y0)<maxFF) % pogledam ali so diagonalne celice ovire - potem ne dovolim prehoda
        %  if(xx<xs && yy>ys  && 1)
        if(loc==10)
              g01=[-1 1]*1/sqrt(2);  % to za silo deluje a je nezveznost
              g00=[-1 0]*1;
              g11=[0  1]*1;
        elseif(loc==01)
              g10=[1 -1]*1/sqrt(2);  % to za silo deluje a je nezveznost
              g11=[1 0]*1;
              g00=[0  -1]*1;
        end
      elseif(FF(x0,y1)>maxFF && FF(x1,y0)>maxFF && FF(x0,y0)<maxFF && FF(x1,y1)<maxFF) % pogledam ali so diagonalne celice ovire - potem ne dovolim prehoda
        if(loc==00)
              g11=[1 1]*1/sqrt(2);  % to za silo deluje a je nezveznost
              g10=[1 0]*1;
              g01=[0  1]*1;
        elseif(loc==11)
              g00=[-1 -1]*1/sqrt(2);  % to za silo deluje a je nezveznost
              g01=[-1 0]*1;
              g10=[0  -1]*1;
        end
      end
...
%      %       % uredim diagonalne prehode mimo dotika zasedenih celic
%       if(FF(x0,y0)>maxFF && FF(x1,y1)>maxFF && FF(x0,y1)<maxFF && FF(x1,y0)<maxFF) % pogledam ali so diagonalne celice ovire - potem ne dovolim prehoda
% %         vect=[xs;ys] -[xx;yy];
% %         grad=rrr(2:3);
% %         fx(ax+sign(xx-xs))=rrr(1) + grad*vect;   % popravek costa za celico, ki ji dolocam gradient v centru
%           
%  
%           if(xx<xs && yy>ys)
%              gx=g10(1);
...
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [g]=getCellGrad(ax_,ay_,x0,y0,xx,yy,rrr, FF,sizeX,sizeY,x_min,y_min,eC,maxFF)
   ...
   xs=(ax-1)*eC+ eC/2 +x_min; ys=(ay-1)*eC+ eC/2+y_min;% sredisce celice, ki ji dolocam gradient
       ...
       elseif(ax<2) % ce je center celice ax<=1 -> sem na robu za gradient od?itam celico levo in celico desno 
           grad=rrr(2:3);  
           vect=[xs-eC;ys] -[xx;yy];
...
%       elseif(FF(x0,y0)>maxFF && FF(x0+1,y0+1)>maxFF) % pogledam ali so diagonalne celice ovire - potem ne dovolim prehoda
%         
%         if( ax==x0 && ay==y0+1 && (xx-xs)>eC/2 )  
% 
%             vect=[xs;ys] -[xx;yy];
%             grad=rrr(2:3);
%             vect2=[xs-eC/2;ys] -[xx;yy];
%             fx(2)=rrr(1) + grad*vect;   % popravek costa za celico, ki ji dolocam gradient v centru
%             fx(1)=rrr(1) + grad*vect2;  % popravek costa za celico, ki ji dolocam gradient v centru
%         end
      end
...
%       elseif(FF(x0,y0)>maxFF && FF(x0+1,y0+1)>maxFF) % pogledam ali so diagonalne celice ovire - potem ne dovolim prehoda
%         
%         if( ax==x0 && ay==y0+1 && (yy-ys)<-eC/2 )  
% 
%             vect=[xs;ys] -[xx;yy];
%             grad=rrr(2:3);
%             vect2=[xs;ys+eC/2] -[xx;yy];
%             fy(2)=rrr(1) + grad*vect;   % popravek costa za celico, ki ji dolocam gradient v centru
%             fy(3)=rrr(1) + grad*vect2;  % popravek costa za celico, ki ji dolocam gradient v centru
%         end
