classdef trajectorys
    ...
    methods
        ...
        function [eeTraj, baseTraj, q0] = trajectoryOrientation(obj, showPlot)
                        
            % initial pose base
            xb0 = [0 0 0]; % x y fi

            % initial pose joints
            q0 = [0 0 0 -pi/2 0 4/5*pi 0 0 0 0];
           ...
           rEE1 = [ 0.9496   -0.0033    0.3135 
            ...
            % EE trajectory
            xtee = 0.5215 + (0:obj.step:obj.totalTime);
            ytee = 0.25 * ones(size(0:obj.step:obj.totalTime));
            ztee = [1.6252 - 0.2252/(obj.totalTime/2) * (0:obj.step:obj.totalTime/2) 1.4 + 0.2252/(obj.totalTime/2) *  (obj.step:obj.step:obj.totalTime/2)]; 
            rotee = rottraj(rEE0, rEE1,[0 obj.totalTime], 0:obj.step:obj.totalTime);

            % create eeTraj - T matrixes
            eeTraj = [];
