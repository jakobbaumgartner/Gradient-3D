...
if flagLoadGrega
    ...
    load Potencialno1
% else
%     load 16-Jan-2023-2-none-0-9152
%     
%     xbr=data.trajBase(:,1); % referenca za bazo ?
%     ybr=data.trajBase(:,2);
%     
%     
%     xar=squeeze( data.trajEE(1,4,:) ) ; % referenca za roko
%     yar=squeeze( data.trajEE(2,4,:) ) ; % referenca za roko
%     zar=squeeze( data.trajEE(3,4,:) ) ; % referenca za roko
%    
% %data.dEE 
%     
%     
%     RecBase=data.results(:,8:10);  % stanja baze
%     
%     RecArm=data.results(:,1:7);  % stanja roke
%    
end


hFig = InitAnimation(10,5);
...
%classical = Classical_optimizer();
      ...
      fs=14; 
      plot(xbr,ybr,'r--'), plot3(xar,yar,zar,'g--') % reference za bazo in roko
    % pregledna slika
      qBase=RecBase(10 ,:); qArm=RecArm(10,:); animateRobot(hFig,[qArm,qBase]',1) ; % izris skeleta    
      qBase=RecBase(end ,:); qArm=RecArm(end,:); animateRobot(hFig,[qArm,qBase]',2) ; % izris skeleta    
      qBase=RecBase(end/2 ,:); qArm=RecArm(end/2,:); animateRobot(hFig,[qArm,qBase]',3) ; % izris skeleta    
      qBase=RecBase(end/2+15 ,:); qArm=RecArm(end/2+10,:); animateRobot(hFig,[qArm,qBase]',4) ; % izris skeleta    
      qBase=RecBase(end/2+30 ,:); qArm=RecArm(end/2+30,:); animateRobot(hFig,[qArm,qBase]',5) ; % izris skeleta    
    ...
    % hitrosti baze  
      figure,plot(RecT,RecU(:,1),RecT,RecU(:,2),'--')
   ...
   if ~isempty(xbr)
      ...
      figure(4),plot(RecT,eee,RecT, ee(:,3) )
      xlabel('$$t$$[s]','interpreter','latex','FontSize',fs)
   ...
   % pogreski roke   
      ...
      figure(6),plot(RecT,RecErrEE(:,1:6))
      xlabel('$$t$$[s]','interpreter','latex','FontSize',fs)
     ...
     % title('arm tracking error')
      ...
      figure(7),plot(RecT,eea1,RecT,eea2)
      xlabel('$$t$$[s]','interpreter','latex','FontSize',fs)
